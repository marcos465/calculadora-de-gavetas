...
/**
 * Responsável por estruturar e renderizar as tabelas de peça/**
 * @fileoverview Componente de Interface de Usuário (UI) responsável por renderizar 
 * a Lista de Corte de Marcenaria, Ferragens e os controles de exportação de dados.
 * @module modules/display/cutListUI
 */
...
export class CutListUI {
    ...
    render({ cuttingList = [], hardwareList = [], state = null }) {
        if (!this.container) {
            ...
            if (!this.container) {
                console.error('[CutListUI.render] Impossível renderizar. O contêiner "#resultContainer" não existe.');
                return;
            }
        }
        ...
        // Renderização limpa e responsiva do contêiner principal
        ...
        // Registra os ouvintes de evento após a injeção do HTML no DOM
        this.attachEventListeners();
    }

    /**
     * Gera a marcação HTML para as linhas da tabela de corte com tratamento numérico e sanitização.
     *
     * @private
    ...
    /**
     * Conecta o ouvinte de evento ao botão de exportação CSV recém-renderizado.
     *
     * @private
     * @returns {void}
     */
        ...
        if (btnExport) {
            btnExport.addEventListener('click', (event) => {
                ...
                if (!Array.isArray(this.currentCuttingList) || this.currentCuttingList.length === 0) {
                    alert('Não há itens na lista de corte para serem exportados.');
                    return;
                }

                // Executa a exportação repassando a lista de corte calculada
                exportToCutListCSV(this.currentCuttingList);
            });
        }
    }
